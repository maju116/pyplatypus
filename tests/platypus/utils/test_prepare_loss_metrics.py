from platypus.utils.prepare_loss_metrics import prepare_loss_function, prepare_metrics, prepare_loss_and_metrics


def test_prepare_loss_function(mocker, monkeypatch):
    mocker.patch("platypus.utils.prepare_loss_metrics.convert_to_snake_case", return_value="loss_function_name")
    monkeypatch.setattr("platypus.utils.prepare_loss_metrics.segmentation_loss.loss_function_name", "loss_function", raising=False)
    assert prepare_loss_function(loss="loss_function_name", n_class=2) == "loss_function"

def test_prepare_metrics(mocker):
    mocker.patch("platypus.utils.prepare_loss_metrics.prepare_loss_function", return_value="duplicated_metric1")
...
def test_prepare_loss_and_metrics(mocker):
    script_path = "platypus.utils.prepare_loss_metrics"
