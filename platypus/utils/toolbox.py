...
def concatenate_binary_masks(binary_mask: np.ndarray, colormap: List[Tuple[int, int, int]], sum: bool = False):
    """
    ...
    if sum:  # TODO to separate function
...
def transform_probabilities_into_binaries(prediction: np.ndarray):
    ...
    if max_prob > 0:
        prediction_binary = np.where(prediction == prediction.max(), 1, 0)
    else:
